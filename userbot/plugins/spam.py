...
async def spam_function(event, crazy, lion, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(lion[0])
    if len(lion) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await crazy.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and crazy.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            crazy = await event.client.send_file(
                event.chat_id, crazy, caption=crazy.text
            )
        ...
        if BOTLOG:
            ...
            crazy = await event.client.send_file(BOTLOG_CHATID, crazy)
            await _lionutils.unsavegif(event, crazy)
        return
    elif event.reply_to_msg_id and crazy.text:
        spam_message = crazy.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    crazy = await event.get_reply_message()
    ...
    await spam_function(event, crazy, lion, sleeptimem, sleeptimet)
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using Lion-Z",
    },
)
async def tmeme(event):
