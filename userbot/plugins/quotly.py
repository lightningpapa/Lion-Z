...
@lionub.lion_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic LionZ.", "{tr}qpic -b LionZ."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "LionZ.Webp"
        img.save(output, "webp")
    else:
        output.name = "LionZ.png"
        img.save(output, "PNG")
    output.seek(0)
...
async def stickerchat(lionquotes):
    ...
    if reply.media and reply.media.document.mime_type in ("mp4"):
        return await edit_or_reply(lionquotes, "`this format is not supported now`")
